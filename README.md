...
<br />
<br />
Once I downloaded the lab files and found out they were multiple Windows Event Log files, I decided to upload them to Splunk because it would be easier to serach and parse the logs than through the native Windows Event Viewer.<br/>
...
<br />
<br />
The second question hinted that the user had manipulated the firewall, so I changed the source in Splunk to the Firewall Event Logs. The first log was suspicious if not malicious as the rule was added was called 'Metasploit C2 Bypass, a popular pentesting tool. However, the user field in the log displayed "User=NOT_TRANSLATED", so I decided to find the user cyberjunkie's Security Identifier (SID), that is unqiue to users. Once I discovered the SID, I compared it to the Modifying User in the firewall log and it proves that the the user cyberjunkie made updated the firewall rule.  <br/>
...
<br />
<br />
The next question mentioned that the user had changed audit policy on the machine and I was to discover what was changed. I researched what Event ID would log audit policy changes and found it to be Event ID 4719, and using SPL with that Event ID returned one log that revealed Other Object Access Events was changed by the user.<br/>
...
<br />
<br />
Next, it was revealed the native anti-virus identified a threat and performed actions on what it found. With that knowledge, I again researched Event IDs related to Windows Defender, and came accross two: 1116 and 1117. Within Splunk changing the source of logs to Windows Defender Operational with the Event ID 1116, I discovered Windows Defender detected the hack tool called SharpHound. With Evend ID 1117, it showed that 100% of actions performed by Windows Defenders were quarantining files and programs meaning that SharpHound was quarintined. <br/>
...
<br />
<br />
The last question mentioned that it is suspected that the malicious user had deleted some logs and I was to find out which ones. I again researched the Event ID for Event Logs being deleted and found it to be Event ID 104, and querying for that in Splunk showed that in within the time of cyberjunkie was logged in, the Microsoft-Windows-Windows Firewall With Advanced Security/Firewall Logs were cleared.<br/>
