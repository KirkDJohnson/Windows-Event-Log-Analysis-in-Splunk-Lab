...
<br />
<br />
The second question hinted that the user has manipulated the firewall, so I changed the source in Splunk to the Firewall Event Logs. The first log was suspicious if not malicious as a rule was added was called 'Metasploit C2 Bypass, a popular pentesting tool. However, the user field in the logs displayed "User=NOT_TRANSLATED", so I decided to find the user cyberjunkie's Security Identifier (SID), that is unqiue to users. Once I discovered the SID, I compared it to the Modifying User in the firewall log and proves that the the user cyberjunkie made updated the firewall rule.  <br/>
